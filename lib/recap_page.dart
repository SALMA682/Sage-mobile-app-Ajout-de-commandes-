...
class RecapPage extends StatefulWidget {
  ...
  const RecapPage({Key? key, required this.commande}) : super(key: key);
...
class _RecapPageState extends State<RecapPage> {
    ...
    try {
      ...
      for (var codeArticle in codesArticles) {
        final artResp = await http.get(
          Uri.parse(
            "http://192.168.1.110:8000/api/articlesDetails/$codeArticle/",
          ),
          headers: {},
        );
      ...
      // Enregistrer en statut "non validée" dans Django
      await _enregistrerCommandeInitiale();
  ...
  Future<void> _enregistrerCommandeInitiale() async {
    try {
      final commandeLocale = Commande(
        dataFormulaire: {
          "entete": widget.commande["entete"],
          "livraison": widget.commande["livraison"],
          "lignes": widget.commande["lignes"],
        },
        clientSage: clientDetails,
        articlesSage: articlesDetails,
        statut: 'non_validee',
      );

      // Insert dans SQLite
      final id = await CommandeService.insertCommande(commandeLocale);

      // Récupère l'ID généré et garde-le pour l'utiliser plus tard
      commandeId = id;

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Commande enregistrée localement avec ID $id")),
      );
    } catch (e) {
      print("Erreur enregistrement local: $e");
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("Erreur enregistrement local")));
    }
  }
  ...
  Future<void> _validerCommande() async {
    ...
    try {
      ...
      if (resp.statusCode == 200 && jsonResp["status"] == "success") {
        ...
        // ❌ Erreurs renvoyées par Sage via Django
        final messages =
            ...
            actions: [
              TextButton(
                onPressed: () {
                  ...
                  Navigator.pop(context); // retour vers formPage
                },
                child: Text("Modifier"),
              ),
            ],
  ...
  Future<void> _ajouterNouvelleCommande() async {
    final confirmed = await _showConfirmationDialog(
      "Nouvelle commande",
      "La commande actuelle sera enregistrée comme non validée. Voulez-vous continuer ?",
    );
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: _ajouterNouvelleCommande,
                          style: ElevatedButton.styleFrom(
                            backgroundColor: Colors.grey,
                          ),
                          child: Text("Ajouter une nouvelle commande"),
                        ),
                      ),
                    ],
                  ),
